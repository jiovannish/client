...
try {
    ...
    if ((& $installed --version) -ne 'jio 0.2.3') { throw 'Installed version mismatch' }
    ...
    if ($LASTEXITCODE -ne 0 -or $version -ne 'jio 0.2.3') { throw 'PowerShell invocation failed' }
    $version = & cmd.exe /d /c '"%JIO_TEST_BINARY%" --version'
    if ($LASTEXITCODE -ne 0 -or $version -ne 'jio 0.2.3') { throw 'CMD invocation failed' }
