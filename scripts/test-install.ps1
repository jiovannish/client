...
try {
    ...
    if ((& $installed --version) -ne 'jio 0.2.4') { throw 'Installed version mismatch' }
    ...
    if ($LASTEXITCODE -ne 0 -or $version -ne 'jio 0.2.4') { throw 'PowerShell invocation failed' }
    $version = & cmd.exe /d /c '"%JIO_TEST_BINARY%" --version'
    if ($LASTEXITCODE -ne 0 -or $version -ne 'jio 0.2.4') { throw 'CMD invocation failed' }
