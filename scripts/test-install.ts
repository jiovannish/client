...
const base = 'https://github.com/jiovannish/client/releases/download/v0.3.0/';
...
`, { mode: 0o755 });
  ...
  for (const [os, arch, target] of [
    ...
    assert.equal(execFileSync(join(bin, 'jio'), ['--version'], { encoding: 'utf8' }), 'jio 0.3.0\n');
  }
  ...
  assert.equal(recovered.stdout, 'jio 0.3.0\n');
